...
{
    (DISPLAY_WIDTH / 2),        // Up
    (DISPLAY_WIDTH / 2) + 36,   // Right
    (DISPLAY_WIDTH / 2),        // Down
    (DISPLAY_WIDTH / 2) - 36    // Left
    ...
    case 0: // Load gfx
    {
        ...
        for (i = 0; i < MAX_REGISTERED_ITEMS; i++) {
            ...
            if (j == WINDOW_NONE)
                continue;
            PutWindowTilemap(j);
            BlitItemIconToWindow(gSaveBlock1Ptr->registeredItems[i], j, i % 2 ? 8 : 4, 8, i == 3 ? sKeyItemWheelExtraPalette : NULL);
...
static void AddBagSortSubMenu(void)
{
    ...
    StringCopy(gStringVar1, gPocketNamesStringsTable[gBagPosition.pocket]);
...
static s8 CompareItemsByName(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2)
{
    ...
    for (i = 0; ; ++i)
    {
        ...
        if ((name1[i] == 85 ? 202 : name1[i]) < (name2[i] == 85 ? 202 : name2[i]))
            return -1;
        else if ((name1[i] == 85 ? 202 : name1[i]) > (name2[i] == 85 ? 202 : name2[i]))
            return 1;
    }
